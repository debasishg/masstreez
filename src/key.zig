...
/// | `""`            |     0 | `0`                          |
pub fn make_slice(key: []const u8, depth: usize) Slice {
...
pub fn slice_count(key: []const u8) usize {
...
test "make_slice: first 8 bytes" {
    const s = make_slice("hello world", 0);
    const expected = mem.readInt(u64, "hello wo", .big);
    try testing.expectEqual(expected, s);
}

test "make_slice: second slice with zero padding" {
    const s = make_slice("hello world", 1);
    var buf: [8]u8 = [_]u8{0} ** 8;
    @memcpy(buf[0..3], "rld");
    try testing.expectEqual(mem.readInt(u64, &buf, .big), s);
}

test "make_slice: short key padded" {
    const s = make_slice("hi", 0);
    var buf: [8]u8 = [_]u8{0} ** 8;
    @memcpy(buf[0..2], "hi");
    try testing.expectEqual(mem.readInt(u64, &buf, .big), s);
}

test "make_slice: empty key" {
    try testing.expectEqual(@as(Slice, 0), make_slice("", 0));
}

test "make_slice: depth beyond key length" {
    try testing.expectEqual(@as(Slice, 0), make_slice("short", 1));
}

test "slice_count" {
    try testing.expectEqual(@as(usize, 0), slice_count(""));
    try testing.expectEqual(@as(usize, 1), slice_count("a"));
    try testing.expectEqual(@as(usize, 1), slice_count("12345678"));
    try testing.expectEqual(@as(usize, 2), slice_count("123456789"));
    try testing.expectEqual(@as(usize, 2), slice_count("hello world"));
}
...
test "make_slice: lexicographic ordering preserved" {
    const a = make_slice("apple___", 0);
    const b = make_slice("banana__", 0);
