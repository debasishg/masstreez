...
pub const InteriorNode = struct {
    ...
    /// convention.
    pub fn findChildIdx(self: *const InteriorNode, ks: KeySlice) usize {
        var i: usize = 0;
    ...
    pub fn insertAt(self: *InteriorNode, pos: usize, sep_key: KeySlice, right_child: ChildPtr) void {
    ...
    pub fn isFull(self: *const InteriorNode) bool {
...
test "InteriorNode: findChildIdx" {
    ...
    try testing.expectEqual(@as(usize, 0), node.findChildIdx(50));
    try testing.expectEqual(@as(usize, 1), node.findChildIdx(100));
    try testing.expectEqual(@as(usize, 1), node.findChildIdx(150));
    try testing.expectEqual(@as(usize, 2), node.findChildIdx(200));
    try testing.expectEqual(@as(usize, 3), node.findChildIdx(300));
    try testing.expectEqual(@as(usize, 3), node.findChildIdx(400));
}

test "InteriorNode: insertAt" {
    var node = InteriorNode{};
    ...
    node.insertAt(0, 100, .{ .leaf = dummy_leaf2 });
    node.insertAt(0, 50, .{ .leaf = dummy_leaf3 });
